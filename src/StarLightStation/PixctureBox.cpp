...
CPixctureBox::CPixctureBox(QWidget *parent) : QWidget(parent)
{
...
void CPixctureBox::setBackground(QBrush brush)
{
	m_brush = brush;
	update();
}

void CPixctureBox::setMode(PB_MODE mode)
{
	m_mode = mode;
	if (m_mode == AUTO_SIZE)
	{
...
bool CPixctureBox::setImage(QImage &image, double scale)
{
	if (image.isNull())
	{
		return false;
...
void CPixctureBox::paintEvent(QPaintEvent * event)
{
	Q_UNUSED(event);
